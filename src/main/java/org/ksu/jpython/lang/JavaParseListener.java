...
import org.ksu.jpython.lang.antlr4.Java8Parser;
import org.ksu.jpython.lang.antlr4.Java8ParserListener;

import java.util.List;

public class JavaParseListener implements Java8ParserListener {

    private final Java8Parser parser;
    private final TranslationBuilder builder;

    public JavaParseListener(Java8Parser parser, TranslationBuilder builder) {
        this.parser = parser;
    ...
    public void exitExpressionName(Java8Parser.ExpressionNameContext ctx) {
        ...
        if (builder.show().contains("[") && !builder.show().contains("]")) {
            String out = "]";
            builder.outputNoTab(out);
        }
    }
    ...
    public void exitExclusiveOrExpression(Java8Parser.ExclusiveOrExpressionContext ctx) {
        if (ctx.parent instanceof Java8Parser.ExclusiveOrExpressionContext){
            builder.outputNoTab(" " + ctx.parent.getChild(1).getText() + " ");
        }
    }
    //RC
